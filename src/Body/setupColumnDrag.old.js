...
export default ({ onDrag = emptyFn } = {}, event) => {
  ...
  DragHelper(event, {
    ...
    onDragStart(event, config) {
      ...
      // const columnHeaders = headerNode.querySelectorAll('.' + props.cellClassName)
      //
      // columnData = props.columns.map((column, i) => {
      //   const region = Region.from(columnHeaders[i])
      //
